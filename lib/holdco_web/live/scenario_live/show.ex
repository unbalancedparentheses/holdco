defmodule HoldcoWeb.ScenarioLive.Show do
  ...
  def handle_event("save_item", _params, %{assigns: %{can_write: false}} = socket) do
    {:noreply, put_flash(socket, :error, "You don't have permission to do that")}
  end

  def handle_event("delete_item", _params, %{assigns: %{can_write: false}} = socket) do
    {:noreply, put_flash(socket, :error, "You don't have permission to do that")}
  end
  ...
  def handle_event("save_item", %{"item" => params}, socket) do
    ...
    case Scenarios.create_scenario_item(params) do
      {:ok, _} ->
        {:noreply, reload(socket) |> put_flash(:info, "Item added") |> assign(show_form: false)}
    ...
    <div class="page-title">
      <div style="display: flex; justify-content: space-between; align-items: flex-start;">
        <div>
          <h1>{@scenario.name}</h1>
          <p class="deck">
            {@scenario.description || "Financial projection"} &mdash; {@scenario.projection_months} months {if @scenario.company,
              do: " for #{@scenario.company.name}"}
          </p>
        </div>
      ...
      <div class="panel">
          ...
          <tbody>
            <%= for item <- @scenario.items do %>
              <tr>
                ...
                <td>
                  <span class={"tag #{if item.item_type == "revenue", do: "tag-jade", else: "tag-crimson"}"}>
                    {item.item_type}
                  </span>
                </td>
        ...
        <%= if @scenario.items == [] do %>
          <div class="empty-state">
            No items yet. Add revenue and expense items to build the projection.
          </div>
        <% end %>
      </div>
    </div>
          ...
          <div class="modal-body">
            <form phx-submit="save_item">
              <div class="form-group">
                ...
                <input type="text" name="item[name]" class="form-input" required />
              </div>
              <div class="form-group">
              ...
              <div class="form-group">
                ...
                <input type="number" name="item[amount]" class="form-input" step="any" value="0" />
              </div>
              <div class="form-group">
                <label class="form-label">Currency</label>
                <input type="text" name="item[currency]" class="form-input" value="USD" />
              </div>
              <div class="form-group">
                <label class="form-label">Growth Rate %</label>
                <input type="number" name="item[growth_rate]" class="form-input" step="any" value="0" />
              </div>
              <div class="form-group">
