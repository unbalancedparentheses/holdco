defmodule HoldcoWeb.BankAccountsLive.Index do
  ...
  def handle_event("save", %{"bank_account" => params}, socket) do
    case Banking.create_bank_account(params) do
      {:ok, _} -> {:noreply, reload(socket) |> put_flash(:info, "Bank account added") |> assign(show_form: false)}
  ...
  defp reload(socket) do
    accounts = Banking.list_bank_accounts()
    total_balance = Banking.total_balance()
    assign(socket, accounts: accounts, total_balance: total_balance)
  end
  ...
  def render(assigns) do
    ...
    <div class="metrics-strip">
      ...
      <div class="metric-cell">
        ...
        <div class="metric-value"><%= @accounts |> Enum.map(& &1.currency) |> Enum.uniq() |> length() %></div>
      </div>
    </div>

    <div class="grid-2">
      <div class="section">
        <div class="section-head"><h2>Balance by Currency</h2></div>
      ...
      <div class="section">
        <div class="section-head"><h2>By Currency</h2></div>
        <div class="panel">
          <table>
            <thead><tr><th>Currency</th><th class="th-num">Count</th><th class="th-num">Total Balance</th></tr></thead>
            <tbody>
              <%= for {currency, accts} <- Enum.group_by(@accounts, & &1.currency) do %>
                <tr>
                  <td><%= currency %></td>
                  <td class="td-num"><%= length(accts) %></td>
                  <td class="td-num"><%= format_number(Enum.reduce(accts, 0.0, fn a, acc -> acc + (a.balance || 0.0) end)) %></td>
                </tr>
              <% end %>
            </tbody>
          </table>
    ...
    <div class="section">
      <div class="section-head"><h2>All Accounts</h2></div>
      <div class="panel">
        <table>
          <thead>
            <tr>
          ...
          <tbody>
            <%= for ba <- @accounts do %>
              <tr>
                ...
                <td><button phx-click="delete" phx-value-id={ba.id} class="btn btn-danger btn-sm" data-confirm="Delete?">Del</button></td>
              </tr>
            <% end %>
          </tbody>
        </table>
            ...
            <form phx-submit="save">
              ...
              <div class="form-group">
                ...
                <select name="bank_account[account_type]" class="form-select">
                  <option value="operating">Operating</option><option value="savings">Savings</option>
                  <option value="escrow">Escrow</option><option value="trust">Trust</option>
                </select>
              </div>
  ...
  defp currency_chart_data(accounts) do
    by_currency =
      ...
      |> Enum.map(fn {cur, accts} -> {cur, Enum.reduce(accts, 0.0, fn a, acc -> acc + (a.balance || 0.0) end)} end)
