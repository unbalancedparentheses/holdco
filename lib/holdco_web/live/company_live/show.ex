defmodule HoldcoWeb.CompanyLive.Show do
  ...
  alias Holdco.{Corporate, Banking, Assets, Documents, Compliance, Finance, Governance}
  alias Holdco.Corporate.Company

  @tabs ~w(overview holdings bank_accounts transactions documents governance compliance financials)
  ...
  def handle_event("save_holding", %{"holding" => params}, socket) do
    ...
    case Assets.create_holding(params) do
      {:ok, _} -> {:noreply, reload_company(socket) |> put_flash(:info, "Holding added") |> assign(show_form: nil)}
  ...
  def handle_event("save_bank_account", %{"bank_account" => params}, socket) do
    ...
    case Banking.create_bank_account(params) do
      {:ok, _} -> {:noreply, reload_company(socket) |> put_flash(:info, "Bank account added") |> assign(show_form: nil)}
      {:error, _} -> {:noreply, put_flash(socket, :error, "Failed to add bank account")}
    end
  end
  ...
  def handle_event("save_transaction", %{"transaction" => params}, socket) do
    ...
    case Banking.create_transaction(params) do
      {:ok, _} -> {:noreply, reload_company(socket) |> put_flash(:info, "Transaction added") |> assign(show_form: nil)}
  ...
  def handle_event("save_document", %{"document" => params}, socket) do
    ...
    case Documents.create_document(params) do
      {:ok, _} -> {:noreply, reload_company(socket) |> put_flash(:info, "Document added") |> assign(show_form: nil)}
      {:error, _} -> {:noreply, put_flash(socket, :error, "Failed to add document")}
    end
  end
  ...
  def handle_event("save_board_meeting", %{"board_meeting" => params}, socket) do
    ...
    case Governance.create_board_meeting(params) do
      {:ok, _} -> {:noreply, reload_company(socket) |> put_flash(:info, "Board meeting added") |> assign(show_form: nil)}
      {:error, _} -> {:noreply, put_flash(socket, :error, "Failed to add board meeting")}
    end
  end
  ...
  def handle_event("save_service_provider", %{"service_provider" => params}, socket) do
    ...
    case Corporate.create_service_provider(params) do
      {:ok, _} -> {:noreply, reload_company(socket) |> put_flash(:info, "Service provider added") |> assign(show_form: nil)}
      {:error, _} -> {:noreply, put_flash(socket, :error, "Failed to add service provider")}
    end
  end
  ...
  def handle_event("save_key_personnel", %{"key_personnel" => params}, socket) do
    ...
    case Corporate.create_key_personnel(params) do
      {:ok, _} -> {:noreply, reload_company(socket) |> put_flash(:info, "Key personnel added") |> assign(show_form: nil)}
      {:error, _} -> {:noreply, put_flash(socket, :error, "Failed to add key personnel")}
    end
  end
  ...
  def handle_event("save_beneficial_owner", %{"beneficial_owner" => params}, socket) do
    ...
    case Corporate.create_beneficial_owner(params) do
      {:ok, _} -> {:noreply, reload_company(socket) |> put_flash(:info, "Beneficial owner added") |> assign(show_form: nil)}
      {:error, _} -> {:noreply, put_flash(socket, :error, "Failed to add beneficial owner")}
    end
  end
  ...
  def handle_event("save_tax_deadline", %{"tax_deadline" => params}, socket) do
    ...
    case Compliance.create_tax_deadline(params) do
      {:ok, _} -> {:noreply, reload_company(socket) |> put_flash(:info, "Tax deadline added") |> assign(show_form: nil)}
      {:error, _} -> {:noreply, put_flash(socket, :error, "Failed to add tax deadline")}
    end
  end
  ...
  def handle_event("save_financial", %{"financial" => params}, socket) do
    ...
    case Finance.create_financial(params) do
      {:ok, _} -> {:noreply, reload_company(socket) |> put_flash(:info, "Financial record added") |> assign(show_form: nil)}
      {:error, _} -> {:noreply, put_flash(socket, :error, "Failed to add financial record")}
    end
  end
  ...
  def handle_event("save_insurance_policy", %{"insurance_policy" => params}, socket) do
    ...
    case Compliance.create_insurance_policy(params) do
      {:ok, _} -> {:noreply, reload_company(socket) |> put_flash(:info, "Insurance policy added") |> assign(show_form: nil)}
      {:error, _} -> {:noreply, put_flash(socket, :error, "Failed to add insurance policy")}
    end
  end
  ...
  def render(assigns) do
    ...
    <div class="tabs">
      <button :for={tab <- ~w(overview holdings bank_accounts transactions documents governance compliance financials)}
        class={"tab #{if @active_tab == tab, do: "tab-active"}"}
        phx-click="switch_tab" phx-value-tab={tab}>
        <%= tab_label(tab) %>
      </button>
    </div>
  ...
  defp render_tab(%{active_tab: "overview"} = assigns) do
    ...
    <div class="grid-2">
      <div class="section">
        <div class="section-head"><h2>Company Details</h2></div>
        <div class="panel" style="padding: 1rem;">
          <dl class="detail-list">
            <div><dt>Legal Name</dt><dd><%= @company.legal_name || @company.name %></dd></div>
            <div><dt>Country</dt><dd><%= @company.country %></dd></div>
            <div><dt>Category</dt><dd><%= @company.category %></dd></div>
            <div><dt>Tax ID</dt><dd><%= @company.tax_id || "---" %></dd></div>
            <div><dt>KYC Status</dt><dd><span class={"tag #{kyc_tag(@company.kyc_status)}"}><%= @company.kyc_status %></span></dd></div>
            <div><dt>Status</dt><dd><span class={"tag #{status_tag(@company.wind_down_status)}"}><%= @company.wind_down_status %></span></dd></div>
            <div><dt>Formation Date</dt><dd><%= @company.formation_date || "---" %></dd></div>
            <div><dt>Website</dt><dd><%= @company.website || "---" %></dd></div>
            <div><dt>Lawyer/Studio</dt><dd><%= @company.lawyer_studio || "---" %></dd></div>
          </dl>
        </div>
      </div>
      ...
      <div class="section">
        <div class="section-head">
          <h2>Key Personnel</h2>
          <button class="btn btn-sm btn-primary" phx-click="show_form" phx-value-form="key_personnel">Add</button>
        </div>
        <div class="panel">
          <table>
            <thead><tr><th>Name</th><th>Title</th><th>Email</th><th></th></tr></thead>
            <tbody>
              <%= for kp <- @company.key_personnel do %>
                <tr>
                  ...
                  <td><%= kp.title %></td>
                  <td><%= kp.email %></td>
                  <td><button phx-click="delete_key_personnel" phx-value-id={kp.id} class="btn btn-danger btn-sm" data-confirm="Delete?">Del</button></td>
                </tr>
              <% end %>
            </tbody>
          </table>
        ...
        <div class="section-head" style="margin-top: 1rem;">
          ...
          <button class="btn btn-sm btn-primary" phx-click="show_form" phx-value-form="beneficial_owner">Add</button>
        </div>
        <div class="panel">
          <table>
            <thead><tr><th>Name</th><th>Nationality</th><th>Ownership %</th><th>Verified</th><th></th></tr></thead>
            <tbody>
              <%= for bo <- @company.beneficial_owners do %>
                <tr>
                  ...
                  <td><button phx-click="delete_beneficial_owner" phx-value-id={bo.id} class="btn btn-danger btn-sm" data-confirm="Delete?">Del</button></td>
                </tr>
              <% end %>
            </tbody>
          </table>
        ...
        <div class="section-head" style="margin-top: 1rem;">
          ...
          <button class="btn btn-sm btn-primary" phx-click="show_form" phx-value-form="service_provider">Add</button>
        </div>
        <div class="panel">
          <table>
            <thead><tr><th>Role</th><th>Name</th><th>Firm</th><th>Email</th><th></th></tr></thead>
            <tbody>
              <%= for sp <- @company.service_providers do %>
                <tr>
                  ...
                  <td><%= sp.firm %></td>
                  <td><%= sp.email %></td>
                  <td><button phx-click="delete_service_provider" phx-value-id={sp.id} class="btn btn-danger btn-sm" data-confirm="Delete?">Del</button></td>
                </tr>
              <% end %>
            </tbody>
          </table>
    ...
    <%= if @company.subsidiaries != [] do %>
      <div class="section">
        <div class="section-head"><h2>Subsidiaries</h2></div>
        <div class="panel">
          <table>
            <thead><tr><th>Name</th><th>Country</th><th>Ownership</th><th>Status</th></tr></thead>
            <tbody>
              <%= for sub <- @company.subsidiaries do %>
                <tr>
                  <td><.link navigate={~p"/companies/#{sub.id}"} class="td-link td-name"><%= sub.name %></.link></td>
                  <td><%= sub.country %></td>
                  <td class="td-num"><%= if sub.ownership_pct, do: "#{sub.ownership_pct}%", else: "---" %></td>
                  <td><span class={"tag #{status_tag(sub.wind_down_status)}"}><%= sub.wind_down_status %></span></td>
                </tr>
              <% end %>
            </tbody>
          </table>
  ...
  defp render_tab(%{active_tab: "holdings"} = assigns) do
    ...
    <div class="section">
      <div class="section-head">
        <h2>Holdings</h2>
        <button class="btn btn-sm btn-primary" phx-click="show_form" phx-value-form="holding">Add Holding</button>
      </div>
      <div class="panel">
        <table>
          <thead><tr><th>Asset</th><th>Ticker</th><th>Qty</th><th>Unit</th><th>Type</th><th>Currency</th><th></th></tr></thead>
          <tbody>
            <%= for h <- @company.asset_holdings do %>
              <tr>
                ...
                <td><%= h.currency %></td>
                <td><button phx-click="delete_holding" phx-value-id={h.id} class="btn btn-danger btn-sm" data-confirm="Delete?">Del</button></td>
              </tr>
            <% end %>
          </tbody>
        </table>
  ...
  defp render_tab(%{active_tab: "bank_accounts"} = assigns) do
    ...
    <div class="section">
      <div class="section-head">
        <h2>Bank Accounts</h2>
        <button class="btn btn-sm btn-primary" phx-click="show_form" phx-value-form="bank_account">Add Account</button>
      </div>
      <div class="panel">
        <table>
          <thead><tr><th>Bank</th><th>Account #</th><th>IBAN</th><th>Type</th><th>Currency</th><th class="th-num">Balance</th><th></th></tr></thead>
          <tbody>
            <%= for ba <- @company.bank_accounts do %>
              <tr>
                ...
                <td><button phx-click="delete_bank_account" phx-value-id={ba.id} class="btn btn-danger btn-sm" data-confirm="Delete?">Del</button></td>
              </tr>
            <% end %>
          </tbody>
        </table>
  ...
  defp render_tab(%{active_tab: "transactions"} = assigns) do
    ...
    <div class="section">
      <div class="section-head">
        <h2>Transactions</h2>
        <button class="btn btn-sm btn-primary" phx-click="show_form" phx-value-form="transaction">Add Transaction</button>
      </div>
      <div class="panel">
        <table>
          <thead><tr><th>Date</th><th>Type</th><th>Description</th><th>Counterparty</th><th class="th-num">Amount</th><th>Currency</th><th></th></tr></thead>
          <tbody>
            <%= for tx <- @company.transactions do %>
              <tr>
                ...
                <td class={"td-num #{if tx.amount && tx.amount < 0, do: "num-negative", else: "num-positive"}"}><%= tx.amount %></td>
                <td><%= tx.currency %></td>
                <td><button phx-click="delete_transaction" phx-value-id={tx.id} class="btn btn-danger btn-sm" data-confirm="Delete?">Del</button></td>
              </tr>
            <% end %>
          </tbody>
        </table>
  ...
  defp render_tab(%{active_tab: "documents"} = assigns) do
    ...
    <div class="section">
      <div class="section-head">
        <h2>Documents</h2>
        <button class="btn btn-sm btn-primary" phx-click="show_form" phx-value-form="document">Add Document</button>
      </div>
      <div class="panel">
        <table>
          <thead><tr><th>Name</th><th>Type</th><th>URL</th><th>Notes</th><th></th></tr></thead>
          <tbody>
            <%= for doc <- @company.documents do %>
              <tr>
                ...
                <td><%= doc.notes %></td>
                <td><button phx-click="delete_document" phx-value-id={doc.id} class="btn btn-danger btn-sm" data-confirm="Delete?">Del</button></td>
              </tr>
            <% end %>
          </tbody>
        </table>
  ...
  defp render_tab(%{active_tab: "governance"} = assigns) do
    ...
    <div class="section">
      <div class="section-head">
        ...
        <button class="btn btn-sm btn-primary" phx-click="show_form" phx-value-form="board_meeting">Add Meeting</button>
      </div>
      <div class="panel">
        <table>
          <thead><tr><th>Date</th><th>Type</th><th>Status</th><th>Notes</th><th></th></tr></thead>
          <tbody>
            <%= for bm <- @company.board_meetings do %>
              <tr>
                ...
                <td><%= bm.notes %></td>
                <td><button phx-click="delete_board_meeting" phx-value-id={bm.id} class="btn btn-danger btn-sm" data-confirm="Delete?">Del</button></td>
              </tr>
            <% end %>
          </tbody>
        </table>
    ...
    <div class="grid-2">
      <div class="section">
        ...
        <div class="panel">
          <table>
            <thead><tr><th>Investor</th><th>Round</th><th>Shares</th><th>Amount</th><th>Date</th></tr></thead>
            <tbody>
      ...
      <div class="section">
        <div class="section-head"><h2>Resolutions</h2></div>
        <div class="panel">
          <table>
            <thead><tr><th>Title</th><th>Type</th><th>Date</th><th>Passed</th></tr></thead>
            <tbody>
    ...
    <div class="grid-2">
      <div class="section">
        <div class="section-head"><h2>Deals</h2></div>
        <div class="panel">
          <table>
            <thead><tr><th>Type</th><th>Counterparty</th><th>Value</th><th>Status</th></tr></thead>
            <tbody>
      ...
      <div class="section">
        <div class="section-head"><h2>Joint Ventures</h2></div>
        <div class="panel">
          <table>
            <thead><tr><th>Name</th><th>Partner</th><th>Ownership</th><th>Status</th></tr></thead>
            <tbody>
    ...
    <div class="section">
      <div class="section-head"><h2>Powers of Attorney</h2></div>
      <div class="panel">
        <table>
          <thead><tr><th>Grantor</th><th>Grantee</th><th>Scope</th><th>Start</th><th>End</th><th>Status</th></tr></thead>
          <tbody>
  ...
  defp render_tab(%{active_tab: "compliance"} = assigns) do
    ...
    <div class="section">
      <div class="section-head">
        <h2>Tax Deadlines</h2>
        <button class="btn btn-sm btn-primary" phx-click="show_form" phx-value-form="tax_deadline">Add Deadline</button>
      </div>
      <div class="panel">
        <table>
          <thead><tr><th>Jurisdiction</th><th>Description</th><th>Due Date</th><th>Status</th><th></th></tr></thead>
          <tbody>
            <%= for td <- @company.tax_deadlines do %>
              <tr>
                ...
                <td><button phx-click="delete_tax_deadline" phx-value-id={td.id} class="btn btn-danger btn-sm" data-confirm="Delete?">Del</button></td>
              </tr>
            <% end %>
          </tbody>
        </table>
    ...
    <div class="grid-2">
      <div class="section">
        <div class="section-head">
          ...
          <button class="btn btn-sm btn-primary" phx-click="show_form" phx-value-form="insurance_policy">Add Policy</button>
        </div>
        <div class="panel">
          <table>
            <thead><tr><th>Type</th><th>Provider</th><th>Coverage</th><th>Expiry</th><th></th></tr></thead>
            <tbody>
              <%= for ip <- @company.insurance_policies do %>
                <tr>
                  ...
                  <td><button phx-click="delete_insurance_policy" phx-value-id={ip.id} class="btn btn-danger btn-sm" data-confirm="Delete?">Del</button></td>
                </tr>
              <% end %>
            </tbody>
          </table>
      ...
      <div class="section">
        <div class="section-head"><h2>Regulatory Filings</h2></div>
        <div class="panel">
          <table>
            <thead><tr><th>Jurisdiction</th><th>Type</th><th>Due</th><th>Status</th></tr></thead>
            <tbody>
    ...
    <div class="grid-2">
      <div class="section">
        <div class="section-head"><h2>Regulatory Licenses</h2></div>
        <div class="panel">
          <table>
            <thead><tr><th>Type</th><th>Authority</th><th>Expiry</th><th>Status</th></tr></thead>
            <tbody>
      ...
      <div class="section">
        <div class="section-head"><h2>ESG Scores</h2></div>
        <div class="panel">
          <table>
            <thead><tr><th>Period</th><th>E</th><th>S</th><th>G</th><th>Overall</th></tr></thead>
            <tbody>
  ...
  defp render_tab(%{active_tab: "financials"} = assigns) do
    ...
    <div class="section">
      <div class="section-head">
        <h2>Financials</h2>
        <button class="btn btn-sm btn-primary" phx-click="show_form" phx-value-form="financial">Add Period</button>
      </div>
      <div class="panel">
        <table>
          <thead><tr><th>Period</th><th class="th-num">Revenue</th><th class="th-num">Expenses</th><th class="th-num">Net</th><th>Currency</th><th></th></tr></thead>
          <tbody>
            <%= for f <- @company.financials do %>
              <tr>
                ...
                <td class={"td-num #{if (f.revenue || 0) - (f.expenses || 0) >= 0, do: "num-positive", else: "num-negative"}"}><%= (f.revenue || 0) - (f.expenses || 0) %></td>
                <td><%= f.currency %></td>
                <td><button phx-click="delete_financial" phx-value-id={f.id} class="btn btn-danger btn-sm" data-confirm="Delete?">Del</button></td>
              </tr>
            <% end %>
          </tbody>
        </table>
    ...
    <div class="grid-2">
      <div class="section">
        ...
        <div class="panel">
          <table>
            <thead><tr><th>Type</th><th>Creditor</th><th class="th-num">Principal</th><th>Rate</th><th>Maturity</th><th>Status</th></tr></thead>
            <tbody>
      ...
      <div class="section">
        <div class="section-head"><h2>Dividends</h2></div>
        <div class="panel">
          <table>
            <thead><tr><th>Date</th><th>Recipient</th><th class="th-num">Amount</th><th>Type</th></tr></thead>
            <tbody>
      ...
      <div class="modal" phx-click-away="close_form">
        ...
        <div class="modal-body">
          <form phx-submit="save_holding">
            <div class="form-group"><label class="form-label">Asset *</label><input type="text" name="holding[asset]" class="form-input" required /></div>
  ...
  defp render_inline_form(%{show_form: "bank_account"} = assigns) do
    ...
    <div class="modal-overlay" phx-click="close_form">
      <div class="modal" phx-click-away="close_form">
        <div class="modal-header"><h3>Add Bank Account</h3></div>
        <div class="modal-body">
          <form phx-submit="save_bank_account">
            <div class="form-group"><label class="form-label">Bank Name *</label><input type="text" name="bank_account[bank_name]" class="form-input" required /></div>
            <div class="form-group"><label class="form-label">Account Number</label><input type="text" name="bank_account[account_number]" class="form-input" /></div>
            <div class="form-group"><label class="form-label">IBAN</label><input type="text" name="bank_account[iban]" class="form-input" /></div>
            <div class="form-group"><label class="form-label">SWIFT</label><input type="text" name="bank_account[swift]" class="form-input" /></div>
            <div class="form-group"><label class="form-label">Currency</label><input type="text" name="bank_account[currency]" class="form-input" value="USD" /></div>
            <div class="form-group">
              <label class="form-label">Account Type</label>
              <select name="bank_account[account_type]" class="form-select">
                <option value="operating">Operating</option><option value="savings">Savings</option><option value="escrow">Escrow</option><option value="trust">Trust</option>
              </select>
            </div>
            <div class="form-group"><label class="form-label">Balance</label><input type="number" name="bank_account[balance]" class="form-input" step="any" value="0" /></div>
            <div class="form-actions">
              <button type="submit" class="btn btn-primary">Add</button>
              <button type="button" phx-click="close_form" class="btn btn-secondary">Cancel</button>
            </div>
  ...
  defp render_inline_form(%{show_form: "document"} = assigns) do
    ...
    <div class="modal-overlay" phx-click="close_form">
      <div class="modal" phx-click-away="close_form">
        ...
        <div class="modal-body">
          <form phx-submit="save_document">
            <div class="form-group"><label class="form-label">Name *</label><input type="text" name="document[name]" class="form-input" required /></div>
            <div class="form-group"><label class="form-label">Type</label><input type="text" name="document[doc_type]" class="form-input" /></div>
      ...
      <div class="modal" phx-click-away="close_form">
        ...
        <div class="modal-body">
          <form phx-submit="save_board_meeting">
            <div class="form-group"><label class="form-label">Scheduled Date *</label><input type="text" name="board_meeting[scheduled_date]" class="form-input" placeholder="YYYY-MM-DD" required /></div>
            <div class="form-group">
              <label class="form-label">Meeting Type</label>
              <select name="board_meeting[meeting_type]" class="form-select">
                <option value="regular">Regular</option><option value="special">Special</option><option value="annual">Annual</option>
              </select>
            </div>
  ...
  defp render_inline_form(%{show_form: "key_personnel"} = assigns) do
    ...
    <div class="modal-overlay" phx-click="close_form">
      <div class="modal" phx-click-away="close_form">
        <div class="modal-header"><h3>Add Key Personnel</h3></div>
        <div class="modal-body">
          <form phx-submit="save_key_personnel">
            <div class="form-group"><label class="form-label">Name *</label><input type="text" name="key_personnel[name]" class="form-input" required /></div>
            <div class="form-group"><label class="form-label">Title</label><input type="text" name="key_personnel[title]" class="form-input" /></div>
            <div class="form-group"><label class="form-label">Email</label><input type="email" name="key_personnel[email]" class="form-input" /></div>
            <div class="form-group"><label class="form-label">Phone</label><input type="text" name="key_personnel[phone]" class="form-input" /></div>
            <div class="form-group"><label class="form-label">Department</label><input type="text" name="key_personnel[department]" class="form-input" /></div>
            <div class="form-actions">
              <button type="submit" class="btn btn-primary">Add</button>
              <button type="button" phx-click="close_form" class="btn btn-secondary">Cancel</button>
            </div>
  ...
  defp render_inline_form(%{show_form: "beneficial_owner"} = assigns) do
    ...
    <div class="modal-overlay" phx-click="close_form">
      <div class="modal" phx-click-away="close_form">
        <div class="modal-header"><h3>Add Beneficial Owner</h3></div>
        <div class="modal-body">
          <form phx-submit="save_beneficial_owner">
            <div class="form-group"><label class="form-label">Name *</label><input type="text" name="beneficial_owner[name]" class="form-input" required /></div>
            <div class="form-group"><label class="form-label">Nationality</label><input type="text" name="beneficial_owner[nationality]" class="form-input" /></div>
            <div class="form-group"><label class="form-label">Ownership %</label><input type="number" name="beneficial_owner[ownership_pct]" class="form-input" step="any" min="0" max="100" /></div>
            <div class="form-group">
  ...
  defp render_inline_form(%{show_form: "service_provider"} = assigns) do
    ...
    <div class="modal-overlay" phx-click="close_form">
      <div class="modal" phx-click-away="close_form">
        <div class="modal-header"><h3>Add Service Provider</h3></div>
        <div class="modal-body">
          <form phx-submit="save_service_provider">
            <div class="form-group"><label class="form-label">Role *</label><input type="text" name="service_provider[role]" class="form-input" required /></div>
            <div class="form-group"><label class="form-label">Name *</label><input type="text" name="service_provider[name]" class="form-input" required /></div>
            <div class="form-group"><label class="form-label">Firm</label><input type="text" name="service_provider[firm]" class="form-input" /></div>
            <div class="form-group"><label class="form-label">Email</label><input type="email" name="service_provider[email]" class="form-input" /></div>
            <div class="form-group"><label class="form-label">Phone</label><input type="text" name="service_provider[phone]" class="form-input" /></div>
            <div class="form-actions">
              <button type="submit" class="btn btn-primary">Add</button>
              <button type="button" phx-click="close_form" class="btn btn-secondary">Cancel</button>
            </div>
      ...
      <div class="modal" phx-click-away="close_form">
        ...
        <div class="modal-body">
          <form phx-submit="save_insurance_policy">
            ...
            <div class="form-group"><label class="form-label">Policy Number</label><input type="text" name="insurance_policy[policy_number]" class="form-input" /></div>
            ...
            <div class="form-group"><label class="form-label">Currency</label><input type="text" name="insurance_policy[currency]" class="form-input" value="USD" /></div>
