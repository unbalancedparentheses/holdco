  ...
  def app(assigns) do
    ...
    <nav class="nav-bar">
      <div class="nav-inner">
        ...
        <% risk_paths = ~w(/anomalies) %>

        <% admin_paths = ~w(/settings /audit-log /alerts /approvals /notifications /import) %>
        <div class="nav-links">
          ...
          <.link navigate={~p"/tax"} class={if String.starts_with?(cur, "/tax"), do: "active"}>Tax</.link>
